...
std::vector< std::string > SpliteStr( std::string const& str, char splitChar ){//按字符分割，返回vector
...
void GpsReader::parseData(){//读取整个databuff,先按$分，再按“,”分，
	...
	for ( std::vector< std::string >::iterator it = dataList.begin(); it != dataList.end(); it ++ ){
		std::vector< std::string > gpsInfoStr = SpliteStr(*it, ',');//按，把每条信息分成若干项 根据信息头（第一项）来判断属于什么格式并进行对应的赋值
...
void GpsReader::parseGPGGA( std::vector< std::string > valueList ){
	...
	if( _first ){
		...
		if((newXYZ-curXYZ).length()<15||(newXYZ-supposedXYZ).length()<15){//新旧距离少于15m 用于筛飞的点
			{
				OpenThreads::ScopedLock<OpenThreads::Mutex> lock(_mutex);
				_lastData = _curData;
				_curData = newData;
	...
	if(m_nGpsUpdateFailCount>7&&(_iCondition==1||_iCondition==2||_iCondition==4||_iCondition==5)){//失败一定次数后车子必定已原离上一次有效数据的位置 再按照距离筛点必定被筛掉 所以要强制录入
		osg::Vec3d curXYZ;
		osgAPEx::ConvertLongLatHeightToXYZ( _curData.getPos().x(), _curData.getPos().y(), _curData.getPos().z(), curXYZ );
		osg::Vec3d newXYZ;
		osgAPEx::ConvertLongLatHeightToXYZ( newData.getPos().x(), newData.getPos().y(), newData.getPos().z(), newXYZ );
