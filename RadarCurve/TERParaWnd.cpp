...
void TERParaWnd::DrawLegend( CDC *lpDC )
{
	...
	CFont cfont;
	cfont.CreateFont( 12, 0, 0, 0, 0, FALSE, FALSE, FALSE , 0, 0, 0, 0, 0, L"宋体");
...
void TERParaWnd::DrawTimeWindow( CDC *lpDC ){
	...
	//起点 x/fullX=delay/fullRange  时窗 x/fullX=(count/ratio*10^9)/(10^6/Frequency)
...
void TERParaWnd::OnPaint()
{
	...
	{
		DrawLegend(&dcMem);//图例
		DrawCoordinateAxis(&dcMem);//坐标轴
		DrawWave(&dcMem);//波形
		DrawTimeWindow(&dcMem);//时窗
	}
	//DrawScaleFlag( &dcMem );
...
void TERParaWnd::DrawBk( CDC *lpDC )//绘制背景   黑底红绿线
{
	CRect rc;
	GetClientRect(&rc);
...
void TERParaWnd::DrawCurve( CDC *lpDC )//绘制波形图
{
	if ( !_drawing )
	{
		return;
...
void TERParaWnd::DrawSection( CDC *lpDC, int newData )//绘制波普数据
{
	if ( !_drawing )
	{
		return;
