...
class RenderToolProfileAnalysisHandler : public osgGA::GUIEventHandler
{
public:
	RenderToolProfileAnalysisHandler(osg::Group* root);
	~RenderToolProfileAnalysisHandler(void);
...
public:
	...
	//设置显示界面
	void setProfileDlg(ProfileAnalysisDlg* profileAnalysisDlg);
	//清除全部测绘
	void clearAll();
	//清除指定对象
...
private:
	...
	*/
	...
	返回：点集
	*/
	std::vector<osg::Vec3f> computerTwoDot(osg::Vec3 beginDot,osg::Vec3 endDot);
	/*
	目的：创建碰面检测阵(框选用）
	参数：	mpld(in)	框选点集
	height(in)      盒子的深度
	...
	返回：盒子指针
	*/
	void createCrashRegion(MPolyDataEmbankment&mpld,float height);
	...
	返回：柱子指针
	*/
	osg::Group* createGengonCylinder3(osg::Vec3Array* dotArray,float height);
	...
	*/
	...
	返回：纹理指针
	*/
	osg::StateSet* createHalfTransparentStateSet();
...
private://控制
	...
	//场景根节点
	osg::Group* m_Root;
	ProfileAnalysisDlg* _profileAnalysisDlg;
