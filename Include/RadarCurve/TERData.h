...
class RADAR_EXPORT TERData : public DBRow
{
public:
	TERData(void);
	~TERData(void);
	...
	float getIndexData( int index, float *t );//按索引取雷达数据值
	...
	void setRecordWheelCount( int value );//设置统计的测量轮道数
	int getRecordWheelCount()const;//返回统计的测量轮道数
...
protected:
	...
	int m_recordWheelCount;//数据中自带的测量轮道数
